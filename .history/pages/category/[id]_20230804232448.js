...
export default function CategoryPage({ category, products }) {
  ...
  function handleChangeFillter(filterName, filterValue) {
          ...
          <FilterWrapper>
            {category.properties.map((property) => (
              <Filter key={property.name}>
                ...
                <select
                  onChange={(e) =>
                    handleChangeFillter(property.name, e.target.value)
                  }
                  value={filterValues.find((f) => f.name === property.name)}
                >
                  <option value="all">{filterValues.value}</option>
