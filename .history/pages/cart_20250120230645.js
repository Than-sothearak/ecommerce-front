...
const CartPage = (shippingFee) => {
  const { cartProducts, addProduct, removeProduct, clearCart, filteredProductCarts  } =
  ...
  console.log(filteredProductCarts)
  ...
  for (const productId of cartProducts) {
    const price = filteredProductCarts.find((p) => p._id == productId)?.price || 0;
    subtotal += price;
  }
           ...
           <Title>Cart</Title>
            ...
            {products?.length > 0 && (
              <Table>
                ...
                <tbody>
                  {filteredProductCarts.map((product) => (
                    <tr key={product._id}>
                      <ProductInfoCell>
                        <ProductImageBox>
