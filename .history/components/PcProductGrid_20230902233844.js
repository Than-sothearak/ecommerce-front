...
export default function PcProductGrid({ products, wishedProduct = [],categories }) {
  ...
  const defaultFilterValues = categories.properties?.map((p) => ({
    name: p.name,
    value: "all",
  }));
  ...
  if (categories) {
    propertiesToFill.push(...categories?.properties);
  }
  console.log(propertiesToFills)
  ...
  return (
    <Container>
    <FilterWrapper>
    {categories.properties?.map((property) => (
